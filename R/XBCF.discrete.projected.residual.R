#' Modified XBCF causal forest for binary treatment variable.
#' @description This function fits XBCF causal forest model with binary treatment variable. In specific, the model is \eqn{y = \tau(X_{con}) + \mu(X_{mod}) \times z + \epsilon, \quad \epsilon\sim N(0, \sigma^2)}. Where the \eqn{\tau(X_{con})} and \eqn{\mu(X_{mod})} are two XBART forest called prognostic and treatment forest respectively.
...
XBCF.discrete.projected.residual <- function(
    ...
    class(obj) <- "XBCFDiscreteProjectedResidual"
    return(obj)
}
