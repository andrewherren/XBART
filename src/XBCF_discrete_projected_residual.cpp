...
Rcpp::List XBCF_discrete_projected_residual_cpp(
    ...
    // define model
    XBCFDiscreteProjectedResidualModel *model = new XBCFDiscreteProjectedResidualModel(kap, s, tau_con, tau_mod, alpha_con, beta_con, alpha_mod, beta_mod, sampling_tau, tau_con_kap, tau_con_s, tau_mod_kap, tau_mod_s);
    model->setNoSplitPenalty(no_split_penalty);

    // State settings
    XBCFDiscreteProjectedResidualState state(&Z_std, Xpointer_con, Xpointer_mod, Xorder_std_con, Xorder_std_mod, N, p_con, p_mod, num_trees_con, num_trees_mod, p_categorical_con, p_categorical_mod, p_continuous_con, p_continuous_mod, set_random_seed, random_seed, n_min, num_cutpoints, mtry_con, mtry_mod, num_sweeps, sample_weights, &y_std, 1.0, max_depth, y_mean, burnin, model->dim_residual, nthread, parallel, a_scaling, b_scaling, N_trt, N_ctrl);
    ...
    mcmc_loop_xbcf_discrete_projected_residual(
        ...
        a_xinfo, b_xinfo, 
        tau_con_xinfo, tau_mod_xinfo, trees_con, trees_mod, 
